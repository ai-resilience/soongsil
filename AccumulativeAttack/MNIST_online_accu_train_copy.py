...
parser.add_argument('--dataroot', default='~/.avalanche/data/mnist', 
                    help='root directory for PermutedMNIST dataset managed by Avalanche')
...
net = MLP(input_size=28*28, hidden_size=512, hidden_layers=1, output_size=10).to(device)
...
test_stream = benchmark.test_stream[0]

trloader = DataLoader(train_stream.dataset, batch_size=args.batch_size, shuffle=True)
teloader = DataLoader(test_stream.dataset, batch_size=args.test_batch_size, shuffle=False)
